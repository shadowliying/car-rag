...
# 获取Langchain的工具链 
def get_qa_chain(llm, vector_store, prompt_template): # 大模型实例，向量数据库（Faiss),生成答案的提示模板
...
if __name__ == "__main__":
    ...
    m3e =  base + "/pre_train_model/m3e-large"        #Embeddings模型
    bge_reranker_large = base + "/pre_train_model/bge-reranker-large"  # Top-k重排模型
    ...
    with open(base + "/data/test_question.json", "r") as f:
        ...
        for idx, line in enumerate(jdata):
            ...
            if(len(faiss_context) > 0):
                faiss_min_score = faiss_context[0][1]  # 格式：(文档内容, 分数)
            cnt = 0
            emb_ans = ""
            for doc, score in faiss_context:
                cnt =cnt + 1
